...
@endif
    ...
    <div class="mb-4">
        ...
        <input type="date" name="release_date" id="release_date" value="{{ old('release_date', isset($album->release_date) ? $album->release_date->format('Y-m-d') : '') }}" required class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:shadow-outline">
        @error('release_date')
            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
        @enderror
    </div>  
